<div class="hero" markdown="1">

# ESP32 教程 + 博客

一个“**教程体系** + **持续更新博客**”的混合站点：  
教程用于沉淀结构化知识，博客用于记录踩坑与最新实践。

<p class="badges" markdown="1">
<span class="badge">📚 系统教程</span>
<span class="badge">✍️ 技术博客</span>
<span class="badge">🔍 全站搜索</span>
<span class="badge">🌓 深色模式</span>
</p>

[从教程开始 →](tutorial/index.md){ .md-button .md-button--primary }
[查看 Blog →](blog/index.md){ .md-button }

</div>

## 教程入口

<div class="cards" markdown="1">

<a class="card" href="tutorial/" markdown="1">
### 🧭 教程
从 0 到 1：环境、Hello、常用调试、工程化。
</a>

<a class="card" href="peripherals/" markdown="1">
### 🔌 外设
GPIO / I2C / SPI / UART… 驱动与常见坑。
</a>

<a class="card" href="network/" markdown="1">
### 🌐 网络
Wi-Fi / MQTT / BLE… 通信与连接方案。
</a>

<a class="card" href="projects/" markdown="1">
### 🧱 项目实战
OTA、传感器采集、日志系统、低功耗。
</a>

</div>

## 最新博客（示例）
- [ESP-IDF menuconfig 常见坑](blog/posts/2026-01-03-menuconfig-pitfalls.md)
- [I2C 扫描与异常处理](blog/posts/2026-01-02-i2c-scan.md)
- [OTA 回滚策略](blog/posts/2026-01-01-ota-rollback.md)

> 建议：教程写“体系化章节”，博客写“即时记录与踩坑”，二者互相链接。
